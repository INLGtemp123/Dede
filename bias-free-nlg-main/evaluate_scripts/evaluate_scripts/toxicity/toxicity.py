...
class ToxicityClassifier(evaluate.Measurement):
    def __init__(self, device = 'cpu'):
        model_name = "/home/gridsan/rlohanimit/fair-nlg/bold/toxic_classifier"
        self.toxic_classifier = pipeline("text-classification", model=model_name, top_k=99999, truncation=True, device = device)
    def compute(self, predictions, aggregation="all", toxic_label="hate", threshold=0.5):
    ...
    #     print("toxic classifier is using " + device)
    #     if self.config_name == "default":
    #         logger.warning("Using default offline toxic classifier checkpoint")
