...
def main():
    ...
    # max_length = max_length - trigger_len
    if stride <= 0:
        stride = max_length
    ...
    for i in tqdm(range(0, encodings.input_ids.size(1), stride)):
        ...
        input_ids = torch.cat([trigger_encodings['input_ids'], encodings.input_ids[:, begin_loc:end_loc]], dim=1).to(device)
        ...
        with torch.no_grad():
            ...
            # print(logits.shape)
            loss_regular = dexperts._get_perplexity(logits=logits[:, trigger_len:], labels=target_ids[:, trigger_len:], exp=False)
            # print(loss_regular)
